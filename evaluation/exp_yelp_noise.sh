...
OUTPUT=results/yelp_noise

SESSION_NAME="yelp_noise_exp"
...
then
  ...
  tmux send-keys -t ${SESSION_NAME} "INPUT='$INPUT';export INPUT;OUTPUT='$OUTPUT';export OUTPUT;SCORING_FUNCTION=2;export SCORING_FUNCTION;./run_yelp_noise.sh" C-m
  tmux rename-window -t ${SESSION_NAME} 'tensor2vec'

  # pte, line
  tmux new-window -n pte_n_line -t ${SESSION_NAME}
  tmux send-keys -t ${SESSION_NAME}:1 "cd evaluation/baseline; INPUT='$INPUT';export INPUT;OUTPUT='$OUTPUT';export OUTPUT;HAS_ZIP=1;export HAS_ZIP; ./run_pte_yelp.sh; ./run_line_yelp.sh" C-m
  ...
  tmux send-keys -t ${SESSION_NAME}:4 "INPUT='$INPUT';export INPUT;OUTPUT='$OUTPUT';export OUTPUT;SCORING_FUNCTION=1;export SCORING_FUNCTION;./run_yelp_noise.sh" C-m
