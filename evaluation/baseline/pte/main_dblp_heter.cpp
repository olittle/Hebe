...
// DBLP data
line_node paper, term, author, venue, year;
line_link link_pt, link_pa, link_pv, link_py;
...
void *TrainModelThread(void *id) {
    ...
    while (1)
    {
        ...
        if(has_year){
          link_py.train_sample(error_vec, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);
        }
...
void TrainModel() {
    ...
    if(has_year){
      sprintf(file_name, "%syear.set", file_path);
      year.init(file_name, vector_size);
    }
    ...
    if(has_year){
      sprintf(file_name, "%spaper_year.net", file_path);
      link_py.init(file_name, &paper, &year, negative);
    }
    ...
    author.output(output_file, binary);
}
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-year", argc, argv)) > 0) has_year = (bool) atoi(argv[i + 1]);
    TrainModel();
    return 0;
}
