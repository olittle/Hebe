...
// Line data
line_node review, term, businame, busiid, zip;
line_link link_rt, link_rbn, link_rbi, link_rz;
bool has_zip = false;
...
void *TrainModelThread(void *id) {
    ...
    while (1)
    {
        ...
        // Yelp data
        link_rt.train_sample(error_vec, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);
        link_rbn.train_sample(error_vec, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);
        link_rbi.train_sample(error_vec, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);
        if(has_zip)
          link_rz.train_sample(error_vec, alpha, gsl_rng_uniform(gsl_r), gsl_rng_uniform(gsl_r), next_random);
...
void TrainModel() {
    ...
    // ====== DBLP ========
    sprintf(file_name, "%sreview.set", file_path);
    review.init(file_name, vector_size);
    sprintf(file_name, "%sterm.set", file_path);
    term.init(file_name, vector_size);
    sprintf(file_name, "%sbusiname.set", file_path);
    businame.init(file_name, vector_size);
    sprintf(file_name, "%sbusiid.set", file_path);
    busiid.init(file_name, vector_size);
    if(has_zip){
      sprintf(file_name, "%szip.set", file_path);
      zip.init(file_name, vector_size);
    }
    
    sprintf(file_name, "%sreview_term.net", file_path);
    link_rt.init(file_name, &review, &term, negative);
    sprintf(file_name, "%sreview_businame.net", file_path);
    link_rbn.init(file_name, &review, &businame, negative);
    sprintf(file_name, "%sreview_busiid.net", file_path);
    link_rbi.init(file_name, &review, &busiid, negative);
    if(has_zip){
      sprintf(file_name, "%sreview_zip.net", file_path);
      link_rz.init(file_name, &review, &zip, negative);
    }
    ...
    busiid.output(output_file, binary);
}
...
int main(int argc, char **argv) {
    ...
    if ((i = ArgPos((char *)"-zip", argc, argv)) > 0) has_zip = (bool) atoi(argv[i + 1]);
    TrainModel();
    return 0;
}
